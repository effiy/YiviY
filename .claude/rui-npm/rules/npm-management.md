...
> **当前状态**：本技能未配置自动消费者。`/rui-npm` 是按需手动调用的工具技能，无管线级依赖。任何"管线级依赖健康"或"安全审计"需求须由独立的 rui-\* 技能承载（参见 rui-npm/SKILL.md 的"与 rui 编排的关系"章节）。如未来新增此类消费方，应在 rui-npm/SKILL.md 的 Borders 章节与本文件同步登记。
