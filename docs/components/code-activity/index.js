 ...
 * 两种加载方式：
 *   A. 独立 include（向后兼容）
 *      docs/index.html 用 <div data-include="components/code-activity/index.html">
 *      加载，本文件会被独立调用 mountDocComponent() 自动挂载到当前脚本旁。
 *
 *   B. 嵌入其他组件（如 intro 的 Introduction 区段）
 *      父组件在加载本脚本前设置 window.__docCodeActivityEmbedded = true，
 *      本文件会跳过自动挂载，并通过 window.DocCodeActivityComponent
 *      把组件对象交给父组件，由父组件在指定 slot 内 Vue.createApp().mount()。
 *
 * 行为契约:
...
/* ─────────────────────────────────────────────────────────────────────
 * 组件 options（data / methods / mounted / beforeUnmount）只定义一次，
 * 独立路径和嵌入路径共享同一份定义，避免逻辑漂移。
 * ───────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────
 * 暴露组件对象，供父组件（intro）以 Vue.createApp().mount(slot) 嵌入挂载。
 *   - 嵌入场景：父组件先设置 window.__docCodeActivityEmbedded = true，
 *     本文件跳过自动挂载，仅暴露对象。
 *   - 独立场景：父组件未设置标记，本文件末尾调用 mountDocComponent() 自动挂载。
 * ───────────────────────────────────────────────────────────────────── */
...
/* 无论独立还是嵌入，都需要把组件同目录 index.css 注入到 <head>
   （独立路径下 mountDocComponent() 内部也会注入，这里再做一次是幂等的） */
if (typeof injectComponentStylesheet === 'function') {
    injectComponentStylesheet();
}

/* 嵌入模式下父组件已经持有挂载责任，这里只暴露对象，不抢着挂 */
